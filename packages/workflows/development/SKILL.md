...
1. Never edit workflow state files directly. Resolve the deterministic tools as specified in `WORKFLOW.md`. Pass `--cwd <targetRepository>` to every tool command. A package manager install is optional. Never block intent capture on one.
